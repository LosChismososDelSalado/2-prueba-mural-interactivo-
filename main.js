...
const GalaxiaCorazones = (() => {
    ...
    // --- MOSTRAR FRASE ---
    function mostrarFraseCorazon(data) {
        const panel = document.getElementById('corazon-frase-panel');
        if (!panel) return;
        ...
        // Mostrar texto
        panel.innerHTML = `
            <div style="font-family:${data.f};font-size:clamp(0.8rem,2.5vw,1.4rem);
                color:${data.c};text-shadow:0 0 18px ${data.c},0 0 6px #fff;
                letter-spacing:.06em;line-height:1.2;margin-bottom:8px;">${data.t}</div>
            <div style="font-size:clamp(0.5rem,1.4vw,0.85rem);color:rgba(255,255,255,.8);
                font-family:'Montserrat',sans-serif;letter-spacing:.04em;">${data.s}</div>
        `;
        panel.style.opacity = '1';
        corazonFraseVisible = true;

        // Ocultar frase después de 4 segundos
        setTimeout(() => {
