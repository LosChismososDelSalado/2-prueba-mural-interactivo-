...
// --- AUDIO AMBIENTE ---
// ══════════════════════════════════════════════════════════════
// AUDIO: el ambiente está APAGADO por defecto.
// Solo se activa cuando se muestra nina-1 (ver activarAmbienteSoloNina1).
// ══════════════════════════════════════════════════════════════
...
let ambientePermitido  = false; // true solo cuando nina-1 está activa

function iniciarAmbiente() {
    if (ambienteIniciado || !ambientePermitido) return;
...
function reanudarAmbiente() {
    if (ambienteIniciado && ambientePermitido) {
        audioAmbiente.play().catch(() => {});
    }
}

// Activar ambiente solo para nina-1 (idx=0)
function activarAmbienteSoloNina1(idx) {
    if (idx === 0) {
        ambientePermitido = true;
        if (!ambienteIniciado) {
            audioAmbiente.play().then(() => { ambienteIniciado = true; }).catch(() => {
        ...
        ambientePermitido = false;
        pausarAmbiente();
    }
}

// NO autoplay al cargar — el ambiente solo suena en nina-1
...
function efectoArquitecta(vfx) {
    ...
    <style>
        ...
        .ing-slot  { background:#111; border:1px solid #333; padding:5px 4px; text-align:center; font-family:'Courier New',monospace;
                     color:#555; font-size:.58rem; letter-spacing:1px; border-radius:3px; }
...
// ─── 7. MAESTRA — tabla periódica nueva (iframe) con nina-7.png de fondo ─
// ── AJUSTES FÁCILES ───────────────────────────────────────────────────────
const NINA7_FONDO_OPACIDAD = 0.18; // opacidad de nina-7.png detrás de la tabla (0=invisible, 1=sólida)
const NINA7_FONDO_GLOW     = 8;    // px de glow/blur sobre nina-7.png (0=ninguno)
const NINA7_IFRAME_OPACIDAD = 0.92; // opacidad del iframe de la tabla (0-1)
// ─────────────────────────────────────────────────────────────────────────
...
function efectoMaestra(vfx) {
    ...
    // ── nina-7.png de FONDO semitransparente ────────────────────
    const ninaBg = document.createElement('img');
    ninaBg.src   = 'assets/nina-7.png';
    ninaBg.style.cssText = `
        position:absolute; inset:0; width:100%; height:100%;
        object-fit:contain; object-position:center;
        opacity:${NINA7_FONDO_OPACIDAD};
        filter: brightness(1.2) drop-shadow(0 0 ${NINA7_FONDO_GLOW}px ${GLOW_COLOR_PRINCIPAL});
        pointer-events:none; z-index:1;
    `;
    root.appendChild(ninaBg);

    // ── iframe con la tabla periódica ────────────────────────────
    const frame = document.createElement('iframe');
    frame.src   = 'tabla_periodica.html';
    frame.style.cssText = `
        position:absolute; inset:0; width:100%; height:100%;
        border:none; background:transparent;
        opacity:${NINA7_IFRAME_OPACIDAD};
        z-index:2; pointer-events:auto;
    `;
    // Hacer fondo del iframe transparente para que se vea la nina detrás
    frame.onload = () => {
        try {
            const doc = frame.contentDocument || frame.contentWindow.document;
            if (doc && doc.body) {
                doc.body.style.background = 'transparent';
                const bd = doc.querySelector('body');
                if (bd) bd.style.background = 'rgba(16,19,24,0.55)';
            }
        } catch(e) {/* cross-origin fallback: el fondo del HTML ya es oscuro */ }
    };
...
// ── COLOR DEL GLOW — cambia aquí para todo el proyecto ──────
// Usa cualquier color CSS: rgba, hsl, hex, etc.
const GLOW_COLOR_PRINCIPAL = 'rgba(255, 200, 50, 1)';   // dorado brillante
const GLOW_COLOR_SECUNDARIO = 'rgba(255, 160, 20, 0.6)'; // dorado cálido secundario
const GLOW_COLOR_NINA_PULSO = 'rgba(255, 210, 80, 1)';  // pulso niña (dorado suave)
...
function aplicarOscuridadBase() {
    ...
    // Niña base — siempre brillante (glow dorado)
    const ninaEl = document.getElementById('glow-nina');
    if (ninaEl) {
        ninaEl.style.transition = 'filter 0.5s ease';
        ninaEl.style.filter     = `brightness(1.6) drop-shadow(0 0 20px ${GLOW_COLOR_NINA_PULSO})`;
...
function aplicarConexion(idx) {
    ...
    profesionistas.forEach(id => {
        ...
        if (esAstronauta) {
            ...
            el.style.opacity    = '1';
            el.style.transition = 'filter 0.6s ease, opacity 0.5s ease';
            el.style.filter     = `brightness(${NINA_BRILLO.conectada}) drop-shadow(0 0 ${glowPx}px ${GLOW_COLOR_PRINCIPAL}) drop-shadow(0 0 ${Math.round(glowPx*0.5)}px ${GLOW_COLOR_SECUNDARIO})`;
        } else {
            el.style.opacity    = '1';
            el.style.transition = 'filter 0.6s ease, opacity 0.5s ease';
            el.style.filter     = `brightness(${NINA_BRILLO.profesionistas})`;
...
function mostrarNina(idx) {
    ...
    // Audio ambiente: solo activo en nina-1 (idx=0)
    activarAmbienteSoloNina1(idx);
