...
function efectoAstronauta(vfx) {
        ...
        <div id="as2-pw" style="position:relative;flex-shrink:0;width:clamp(100px,25vw,170px);height:clamp(100px,25vw,170px);">
            ...
            <div id="as2-fw" style="width:100%;height:100%;border-radius:50%;display:none;
                ...
                <img id="as2-fimg" src="assets/balon-tierra.png" style="width:88%;height:88%;
                    object-fit:contain;animation:as2BalonSpin 8s linear infinite;transform-origin:center;border-radius:50%;"
                    onerror="this.src='assets/centro-balon-loader.png'">
            </div>
            <!-- Anillo Saturno -->
    ...
    // ── SONIDO ESPACIAL GENERADO CON WEB AUDIO ──────────────────
    var _spaceAc  = null;
    var _spaceSnd = null;

    function startSpaceSound() {
        try {
            _spaceAc = new (window.AudioContext || window.webkitAudioContext)();
            var master = _spaceAc.createGain();
            master.gain.value = 0;
            master.connect(_spaceAc.destination);
            // Fade in suave
            master.gain.setTargetAtTime(0.18, _spaceAc.currentTime, 1.5);

            // 1. Ruido blanco filtrado (viento estelar)
            var bufLen  = _spaceAc.sampleRate * 4;
            var buf     = _spaceAc.createBuffer(1, bufLen, _spaceAc.sampleRate);
            var data    = buf.getChannelData(0);
            for (var i = 0; i < bufLen; i++) data[i] = (Math.random() * 2 - 1) * 0.4;
            var noise   = _spaceAc.createBufferSource();
            noise.buffer = buf; noise.loop = true;
            var noiseF  = _spaceAc.createBiquadFilter();
            noiseF.type = 'bandpass'; noiseF.frequency.value = 180; noiseF.Q.value = 0.3;
            var noiseG  = _spaceAc.createGain(); noiseG.gain.value = 0.35;
            noise.connect(noiseF); noiseF.connect(noiseG); noiseG.connect(master);
            noise.start();

            // 2. Tono grave pulsante (motor de nave)
            var drone = _spaceAc.createOscillator();
            drone.type = 'sine'; drone.frequency.value = 48;
            var droneG = _spaceAc.createGain(); droneG.gain.value = 0.22;
            // LFO de amplitud para el pulso
            var lfo    = _spaceAc.createOscillator();
            lfo.type   = 'sine'; lfo.frequency.value = 0.18;
            var lfoG   = _spaceAc.createGain(); lfoG.gain.value = 0.18;
            lfo.connect(lfoG); lfoG.connect(droneG.gain);
            drone.connect(droneG); droneG.connect(master);
            drone.start(); lfo.start();

            // 3. Pitidos distantes de estrellas (destellos aleatorios)
            function estrella() {
                if (!_spaceAc || _spaceAc.state === 'closed') return;
                try {
                    var o = _spaceAc.createOscillator();
                    var g = _spaceAc.createGain();
                    o.type = 'sine';
                    // Frecuencia aleatoria alta (como una señal lejana)
                    o.frequency.value = 600 + Math.random() * 1400;
                    g.gain.setValueAtTime(0, _spaceAc.currentTime);
                    g.gain.linearRampToValueAtTime(0.06, _spaceAc.currentTime + 0.08);
                    g.gain.exponentialRampToValueAtTime(0.001, _spaceAc.currentTime + 0.5);
                    o.connect(g); g.connect(master);
                    o.start(_spaceAc.currentTime);
                    o.stop(_spaceAc.currentTime + 0.55);
                } catch(e) {}
                // Próxima estrella en tiempo aleatorio (2-7 segundos)
                setTimeout(estrella, 2000 + Math.random() * 5000);
            }
            setTimeout(estrella, 1000);

            // 4. Whoosh suave de estrella pasando (cada 6-12s)
            function whoosh() {
                if (!_spaceAc || _spaceAc.state === 'closed') return;
                try {
                    var bufW  = _spaceAc.createBuffer(1, _spaceAc.sampleRate * 1.5, _spaceAc.sampleRate);
                    var dataW = bufW.getChannelData(0);
                    for (var j = 0; j < dataW.length; j++) dataW[j] = (Math.random()*2-1)*0.5;
                    var srcW  = _spaceAc.createBufferSource();
                    srcW.buffer = bufW;
                    var filtW = _spaceAc.createBiquadFilter();
                    filtW.type = 'bandpass'; filtW.frequency.value = 900; filtW.Q.value = 0.8;
                    var gW    = _spaceAc.createGain();
                    gW.gain.setValueAtTime(0, _spaceAc.currentTime);
                    gW.gain.linearRampToValueAtTime(0.12, _spaceAc.currentTime + 0.3);
                    gW.gain.linearRampToValueAtTime(0, _spaceAc.currentTime + 1.4);
                    srcW.connect(filtW); filtW.connect(gW); gW.connect(master);
                    srcW.start(); srcW.stop(_spaceAc.currentTime + 1.5);
                } catch(e) {}
                setTimeout(whoosh, 6000 + Math.random() * 6000);
            }
            setTimeout(whoosh, 3000);

            _spaceSnd = { master, noise, drone, lfo };
        } catch(e) {}
    }
    ...
    function stopSpaceSound() {
        if (!_spaceAc) return;
        try {
            if (_spaceSnd && _spaceSnd.master) {
                _spaceSnd.master.gain.setTargetAtTime(0, _spaceAc.currentTime, 0.5);
            }
            setTimeout(function() {
                try { _spaceAc.close(); } catch(e) {}
                _spaceAc = null; _spaceSnd = null;
            }, 800);
        } catch(e) { _spaceAc = null; _spaceSnd = null; }
    }

    // Iniciar sonido (requiere interacción previa del usuario — ya la hubo al tocar la niña)
    setTimeout(startSpaceSound, 200);
    ...
    return {cleanup:function(){
        ...
        if(_elNina2){_elNina2.style.zIndex='';_elNina2.style.position='';}
        vfx.innerHTML='';
    }};
}
