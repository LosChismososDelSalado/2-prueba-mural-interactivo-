...
function efectoFutbolista(vfx) {
    ...
    // FIX 2: stadiumDiv con z-index bajo para que quede detrás de los profesionistas
    const stadiumDiv = document.createElement('div');
    stadiumDiv.style.cssText = `position:absolute;inset:0;overflow:hidden;pointer-events:none;z-index:50;
    ...
    // Cancha verde: posicionada en la franja inferior visible del contenedor
    ...
    field.style.cssText = `
        ...
        bottom:0%;
        z-index:4;
        transform:perspective(900px) rotateX(68deg) translateZ(0px);
        transform-origin:center bottom;
        background:
            repeating-linear-gradient(
                90deg,
                rgba(31,106,55,0.95) 0px,
                rgba(31,106,55,0.95) 90px,
                rgba(44,138,73,0.95) 90px,
                rgba(44,138,73,0.95) 180px
            );
        box-shadow:0 0 120px rgba(0,255,120,.32),inset 0 0 40px rgba(255,255,255,.08);
        animation:ftField 6s linear infinite;
    `;
    ...
    fl.style.cssText = `
        ...
        background:
            ...
            radial-gradient(circle,transparent 0 115px,rgba(255,255,255,.32) 118px,transparent 121px)
            center center/100% 100% no-repeat;
        opacity:.42;
    `;
    field.appendChild(fl);
    stadiumDiv.appendChild(field);
 
    const label = document.createElement('div');
    label.style.cssText = `
        position:absolute;
        top:clamp(5px,1.5%,12px);left:50%;
        transform:translateX(-50%);
        z-index:70;color:white;
        background:rgba(255,255,255,.09);
        border:1px solid rgba(255,255,255,.2);
        backdrop-filter:blur(8px);
        padding:clamp(3px,0.8%,7px) clamp(8px,2%,18px);
        border-radius:20px;
        font-size:clamp(8px,1.6vw,13px);
        letter-spacing:2px;white-space:nowrap;
        font-family:Arial,sans-serif;pointer-events:none;
        animation:ftGlow 2s infinite alternate;
    `;
    label.textContent = '⚽ Estadio Azteca 1971 ⚽';
    stadiumDiv.appendChild(label);
    vfx.appendChild(stadiumDiv);
 
    // CANVAS — z-index entre fondo y profesionistas
    const canvas = document.createElement('canvas');
    canvas.style.cssText = `position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:auto;z-index:51;`;
    ...
    function stopAmbient() {
        if (!ambientFt) return;
        try {
            ambientFt.g.gain.setTargetAtTime(0, _fac.currentTime, 0.3);
            setTimeout(() => { try { ambientFt.src.stop(); } catch(e) {} ambientFt = null; }, 400);
        } catch(e) { ambientFt = null; }
    }
    ...
    function initSnow() {
        ...
        for (let i = 0; i < n; i++)
            snow.push({ x:Math.random()*W, y:Math.random()*H, r:.7+Math.random()*2, sp:.35+Math.random()*.7, dx:(Math.random()-.5)*.35, op:.25+Math.random()*.55 });
    }
    function drawSnow() {
        snow.forEach(s => {
            s.y += s.sp; s.x += s.dx;
    ...
    function spawn() {
        ...
        for (let i = 0; i < n; i++) {
            ...
            do {
                ...
                ok = balls.every(b => Math.hypot(b.x-x, b.y-y) > r+b.r+4); t++;
            } while (!ok && t < 100);
    ...
    // FIX 3: fragmentos de vidrio con física de rebote y fricción mínima
    function explodeBall(b) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            parts.push({
                ...
                rsp: (Math.random()-.5)*.14,
                size: b.r*(.28+Math.random()*.3),
                // FIX 3: sin life limitada — los shards rebotan indefinidamente
                life: 1,
                bouncing: true   // flag para saber que este fragmento rebota
            });
        }
    ...
    function tap(e) {
        ...
        for (const b of balls) {
            if (!b.alive) continue;
            if (Math.hypot(b.x-x, b.y-y) < b.r+8) { explodeBall(b); hit = true; break; }
        }
        if (!hit) {
            balls.forEach(b => {
                if (!b.alive) return;
                ...
                if (d < b.r*4.5) {
                    const f = (b.r*4.5-d)/(b.r*4.5);
                    b.vx += (dx/(d||1))*f*11; b.vy += (dy/(d||1))*f*11;
                    const sp = Math.hypot(b.vx, b.vy); if (sp > 6) { b.vx = (b.vx/sp)*6; b.vy = (b.vy/sp)*6; }
                    playKick();
                }
            });
 
            // FIX 3: el tap también interactúa con los shards — los empuja
            parts.forEach(p => {
                if (p.type !== 'shard' || !p.bouncing) return;
    ...
    function physics() {
            ...
            for (let j = i+1; j < alive.length; j++) {
                ...
                if (d < md && d > .01) {
                    ...
                    if (ad-bd > 0) {
                        ...
                        a.vx = (a.vx/sa)*ta; a.vy = (a.vy/sa)*ta;
                        b.vx = (b.vx/sb)*tb; b.vy = (b.vy/sb)*tb;
        ...
        // Colisión shards ↔ balones vivos: los vidrios rebotan al tocar un balón
        parts.forEach(p => {
            if (p.type !== 'shard' || !p.bouncing) return;
            alive.forEach(b => {
                const dx = p.x - b.x, dy = p.y - b.y;
                const d = Math.hypot(dx, dy);
                const minD = b.r + p.size * 0.8; // radio de colisión del shard
                if (d < minD && d > 0.01) {
                    // Normal de colisión
                    const nx = dx / d, ny = dy / d;
                    // Empujar shard fuera del balón
                    p.x = b.x + nx * (minD + 1);
                    p.y = b.y + ny * (minD + 1);
                    // Reflejar velocidad del shard respecto a la normal + impulso del balón
                    const dot = p.vx * nx + p.vy * ny;
                    p.vx = (p.vx - 2 * dot * nx) * 0.82 + b.vx * 0.4;
                    p.vy = (p.vy - 2 * dot * ny) * 0.82 + b.vy * 0.4;
                    // Velocidad mínima para que siga moviéndose después del choque
                    const spd = Math.hypot(p.vx, p.vy);
                    if (spd < 1.5) { p.vx = nx * 1.5; p.vy = ny * 1.5; }
                    // Girar el shard al impactar
                    p.rsp = (Math.random() - 0.5) * 0.25;
                    // Pequeño efecto sobre el balón (masa mucho mayor, impulso leve)
    ...
    // FIX 3: drawParts con lógica de rebote para shards
    function drawParts() {
        parts.forEach(p => {
            p.x += p.vx; p.y += p.vy;
 
            if (p.type === 'shard' && p.bouncing) {
                // Gravedad leve
                p.vy += 0.04;
 
                // Fricción mínima para mantener movimiento
                p.vx *= 0.998;
                p.vy *= 0.998;
 
                // Rebote en bordes — conserva ~85% de energía
                if (p.x < p.size) { p.x = p.size; p.vx = Math.abs(p.vx) * 0.85; }
                if (p.x > W - p.size) { p.x = W - p.size; p.vx = -Math.abs(p.vx) * 0.85; }
                if (p.y < p.size) { p.y = p.size; p.vy = Math.abs(p.vy) * 0.85; }
                if (p.y > H - p.size) { p.y = H - p.size; p.vy = -Math.abs(p.vy) * 0.85; }
 
                p.rot += p.rsp;
 
                // Mantiene life en 1 (visible) mientras siga en movimiento
                // Solo desaparece si la velocidad es casi cero (parado)
                const speed = Math.hypot(p.vx, p.vy);
                if (speed < 0.02) {
                    p.life -= 0.005; // desvanece lentamente si se detiene
                }
 
            } else {
                // Partículas normales (dots) — física original con decaimiento
                p.vy += 0.12;
                p.life -= p.dec;
            }
            ...
            if (p.type === 'shard') {
                ...
                ctx.fillStyle = p.c;
                ctx.beginPath();
                ctx.arc(p.x, p.y, Math.max(.5, p.r*p.life), 0, Math.PI*2);
                ctx.fill();
            }
        });
        ...
        balls.filter(b => b.alive).forEach(b => {
            ...
            if (balonImg.complete && balonImg.naturalWidth) {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    ctx.beginPath();
                    ctx.arc(Math.cos(a)*b.r*.55, Math.sin(a)*b.r*.55, b.r*.22, 0, Math.PI*2);
                    ctx.fill();
                }
            }
    ...
    return {
        cleanup: () => {
            ...
            if (elBombera) {
                elBombera.style.display = ''; // restaura visibilidad original
            }
        }
    };
}
